...
df_factura, df_tesoreria, df_ordenes, df_edocuenta, df_fact_compra, df_gastos = cargar_datos_general(ruta_archivo)

# --- MENÚ SUPERIOR VISIBLE (SELECCIÓN DE ÁREA) ---
st.sidebar.title("🏢 Grupo SERVYRE")
st.sidebar.markdown("---")
area_seleccionada = st.sidebar.radio("Seleccione el Área:", ["💰 Finanzas", "📚 Contabilidad"], key="area_ppal_radio")

st.sidebar.markdown("---")

if area_seleccionada == "💰 Finanzas":
    menu_fin = st.sidebar.radio("Módulos Financieros:", ["Panel", "Facturación", "OC y SP", "Reporte Pagos"], key="menu_fin_radio")
    ...
    if menu_fin == "Panel":
        st.title("Grupo SERVYRE — Panel Ejecutivo y Consolidado General")
        ...
        if df_ordenes is not None and not df_ordenes.empty:
            ...
            if df_fact_compra is not None:
                ...
                for cand in ['Solicitud de Pago', 'SolicitudPago', 'DocFolio']:
                    if cand in df_fact_compra.columns: col_llave_fc = cand; break
        ...
        if df_tesoreria is not None and not df_tesoreria.empty:
            ...
            if df_gastos is not None:
                ...
                for candidate in ['CFDIFolioFiscal', 'UUID', 'FolioFiscal']:
                    if candidate in df_gastos.columns: col_folio_gs = candidate; break
                ...
                for cand in ['SolicitudPago', 'Solicitud de Pago', 'DocFolio']:
                    if cand in df_gastos.columns: col_llave_gs = cand; break
...
elif area_seleccionada == "📚 Contabilidad":
    menu_con = st.sidebar.radio("Módulos Contables:", ["Polizas", "Balanza de Comprobación", "Conciliación Bancaria"], key="menu_con_radio")
