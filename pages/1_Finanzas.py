...
def cargar_datos_facturacion(path):
    ...
    try:
        ...
        # 1. Cargar FacturaCliente (Contiene Facturas y Notas de Crédito)
        ...
        if not df_fac.empty:
            ...
            # Clasificar TIPO DOC revisando ModuleName
        ...
        # 2. Cargar EdoCuenta y filtrar Amount != 0
...
if df_facturacion_raw is not None and not df_facturacion_raw.empty:
    ...
    # Cruce directo con EdoCuenta (Muestra 2 o más renglones si tiene múltiples pagos)
    if df_edocuenta_raw is not None and not df_edocuenta_raw.empty and 'DocumentID' in df_f.columns and 'DocumentID' in df_edocuenta_raw.columns:
        ...
        # Suma total acumulada por documento para calcular SaldoFactura
        ...
        # Merge directo que genera múltiples renglones si hay varios pagos
    ...
    st.warning("No hay registros disponibles en FacturaCliente.")